...
let tip = d3.tip().attr('class', 'd3-tip').html(function(d) { return formatDateShow(d.Date) + "<br><strong>Pollution Level = " + Math.floor(d.Level*10) / 10 + "</strong><br>NO2 = "+d.NO2; });
...
d3.csv("per_month_kerbside.csv",(err,data_csv) => {
	...
	let scale_y_aq = d3.scale.linear().range([height,0]).domain([min_NO2/25,max_NO2/25]);
